...
@dataclass
class DataCollatorForLMWithRandomShortSeq(DataCollatorMixin):
    """
    ...
    random_short_seq: bool = True
    short_seq_probability: float = 0.10
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        examples_to_cut = torch.where(torch.rand(len(examples)) < 0.9)
        for idx in examples_to_cut[0]:
            target_seq_len = random.randint(2, self.max_seq_length)
            examples[idx]['input_ids'] = examples[idx]['input_ids'][:target_seq_len]
